layui.use(['layer','laytpl', 'element','jquery','form','laypage','laydate'], function(){
  ...
  var form=layui.form,$=layui.jquery;window.tableTpl = tableTpl.innerHTML;
    ...
    window.options=[10,20,50,100]; if(options.indexOf(_pageSize)==-1){ options.push(_pageSize);  function sorter(m,n){if(m < n){return -1;} if(m > n){return 1;} if(m==n){return 0;} } options.sort(sorter);   }

	auto_initDataGrid(urlStr,_startPageNum);// 页面打开，初始化数据表格第一页

	$('#searchForm').find('.layui-btn-group .searchItemMore').on('click',function() {
		if ($(this).hasClass('moreClose')) {
			$(this).removeClass('moreClose');
			$(this).addClass('moreOpen');
			$(this).find('i.layui-icon').html('&#xe61a;');
            $('#searchForm .moreSearchItem')[0].style.display='inline-block';
		...
		else
		{
			...
			$(this).find('i.layui-icon').html('&#xe619;');
			$('#searchForm .moreSearchItem')[0].style.display='none';
			if($(this).parent().parent().hasClass('searchBtGroupPC'))
			{
				var bt = $(this).parent().parent().next().find('.searchItemMore');
		...
		if(typeof resetDataTableHeight === "function" ){
...
var auto_initDataGrid = function(urlStr,currentPage){
    ...
    var skeyValue = ["current=" + currentPage + "&size=" + _pageSize];
    $.each($("#searchForm").find(":input"),function(i,v){
        if($.trim($(v).prop('name')) && $.trim($(v).prop('value')))
            skeyValue.push($.trim($(v).prop('name')) + "=" + $.trim($(v).prop('value')));
    });
    urlStr += urlStr.indexOf("?") > 0 ? "&" + skeyValue.join("&") : "?" + skeyValue.join("&");
	$.get({
		url:encodeURI(encodeURI(urlStr)),
		async:true,
		success:function(data){
			if(!data.page){
                layer.alert("獲取數據失敗:"+((data.msg) ? data.msg : ""),{icon:5});
                tableGrid.innerHTML = "";
                return ;
			}
			var page = data.page;
			page.pageerOptions = options;
			page.checkbox=(!window.hasCheckbox) ? false : window.hasCheckbox;
			var allWidth = 0;
			var tableGridWidth = page.checkbox ? $('#tableGrid').width()*0.99-49 : $('#tableGrid').width()*0.99;
			...
			laytpl(tableTpl).render(page, function(html){tableGrid.innerHTML = html;});
			layui.form.render();
		...
		error:function(){
            tableGrid.innerHTML = "";
			layer.alert("獲取數據失敗",{icon:5});
		}
	});
}
// 页面加载完成后，搜索域内，日期元素绑定事件
function initDateInputForSearchForm(){
	$.each($('#searchForm input.layui-input-date'),function(){
	...
	$.each($('#searchForm input.layui-input-datetime'),function(){
		layui.laydate.render({
			elem: this
			, type: 'datetime'
			, theme: 'molv'
...
var childClick = function(obj){
	...
	if(typeof childClickCall === "function")
    	childClickCall($(obj));
}
var parentClick = function(obj)
{
    ...
    if(typeof parentClickCall === "function")
        parentClickCall($(obj));
}
...
{
	if(width===undefined || width > window.screen.width)width=$(document).width()*0.9;
	if(height===undefined || height > window.screen.height)height=$(document).height()*0.9;
...
var del = function(id)
{
	layui.layer.confirm("您確定刪除該記錄？",{icon: 3, title:'提示'},function(index){
		...
		$.post({
			...
			success:function(data)
			{
				if(data.action==="success"){
                    reload();
                    layui.layer.alert("操作成功",{icon: 1, title:'提示',time:1500});
				}
				else
...
var reload = function(){
    auto_initDataGrid(urlStr,_pageNum);
}
